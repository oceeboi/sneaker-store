...
export async function GET() {
  ...
  const found_address = await Address.findOne({ userId: user_id, type: AddressType.BILLING })
    .select(
  ...
  if (!found_address) {
    return err('Billing address not found', 404);
  }
...
export async function PUT(req: NextRequest) {
  return upsert_typed_address(req, AddressType.BILLING);
}
...
async function upsert_typed_address(req: NextRequest, address_type: AddressType) {
  ...
  Notification.create({
    ...
    message:
      upsert_result.operation_type === 'added'
        ? 'A billing address was added to your account.'
    ...
    console.error('[Notification] billing address alert failed:', notification_error);
  });
